...
describe "Joining channel" do
  ...
  test "join as registered user but not organizer"do
   ...
   {:ok, %Account{} = account} = Accounts.create_account(params)
    ...
    {:ok, _, socket} = socket(QuizaarWeb.UserSocket)
  ...
  test "join as a guest"do
   ...
   {:ok, %Account{} = account} = Accounts.create_account(params)
    ...
    quiz = Factory.insert(:quiz, join_code: "handletest", user_id: user.id, user: user, question_started_at: DateTime.utc_now(), question_time_limit: 30)

    {:ok, _, socket} = socket(UserSocket)
          |> subscribe_and_join(QuizChannel, "quiz:#{quiz.join_code}", %{token: token})
       {:ok, _, player_socket} = socket(UserSocket)
          |> subscribe_and_join(QuizChannel, "quiz:#{quiz.join_code}", %{name: "anyone"})
    {:ok, socket: socket, quiz: quiz, user: user, account: account, player_socket: player_socket}
  ...
  #  end
   ...
   test "handle_in/3 generate_questions failed you are not org", %{player_socket: player_socket} do
      # Simulate the push event to generate questions
      ref = push(player_socket, "generate_questions", %{
          "number" => 5,
        "topic" => "Unit testing",
          "description" => "Testing the quiz generation",
          "difficulty"=> "medium"
...
test "handle_in/3 start_quiz when not organizer", %{player_socket: player_socket
} do
...
describe "Player actions" do
  setup do
    ...
    {:ok, socket: socket, quiz: quiz, user: user, account: account, question: question.data, player_socket: player_socket}
  end
